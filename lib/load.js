...
function loadIsConfig(itemsense, configToLoad, newFacility){
  try{
    ...
    if(!configToLoad) throw new Error("Passed config object is null");
    if(typeof configToLoad === 'string'){
      console.log("passed config in string format, attempt to convert to JSON object")
      configToLoad = JSON.parse(configToLoad);
    }
