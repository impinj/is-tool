        ...
        switch (endpoint) {
          ...
          case 'readerDefinitions':
            ...
            configToLoad.readerDefinitions.forEach((configDefinition) => {
              const readerDefinition = extend({}, configDefinition);
              if (newFacility) {
                readerDefinition.facility = newFacility;
              }
              requestPromises.push(itemsense.readerDefinitions.update(readerDefinition));
            });
            break;
          ...
          case 'users':
            ...
            configToLoad.users.forEach((configDefinition) => {
              const user = extend({}, configDefinition);
              if (addPassword) {
                console.log(`Adding default password of 'default01' to user: ${user.name}`);
                user.password = 'default01';
              }
              requestPromises.push(itemsense.users.update(user));
            });
            break;
          default:
