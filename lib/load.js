...
const { isNonEmptyArray, isInt, createIdToNameMap } = helpers;
...
function replaceAntennaConfIdsWithName(opts) {
  if (isNonEmptyArray(opts.config.thresholds)
    && isNonEmptyArray(opts.config.antennaConfigurations)) {
...
function loadIsConfig(itemsense, newConfigToLoad, newFacility, addPassword) {
  ...
  .then((opts) => {
    if (opts.readersWithFeatures) {
      console.log(`Warning: ReaderDefinitions ${JSON.stringify(opts.readersWithFeatures)} had`,
      ' reader features defined on them.');
      console.log('is-tool has removed the features from these readerDefinitions during load as they',
      ' must be re-enabled manually. You can do this via the IMC or the API.');
    }
    return opts;
  });
}
