...
function addDefaultPasswordToUsers(configurations) {
  if ('users' in configurations) {
    for (const index in configurations.users) {
      if (Object.hasOwnProperty.call(configurations.users, index)) {
        const user = configurations.users[index];
...
function convertR4ToR6(configurations) {
  ...
  try {
    ...
    addDefaultPasswordToUsers(configurations);
