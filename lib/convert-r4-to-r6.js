...
function convertRecipes(readerConfToRecipe, configurations){
  ...
  var add2map = (readerConfNameFromRecipe, recipe) => {
    ...
    if(readerConfNameFromRecipe_l in readerConfToRecipe){
      ...
      readerConfToRecipe[readerConfNameFromRecipe_l] = { recipes: [recipe], consistent: false, type: "", readerConf: {} };
    }
  };
  ...
  if (recipesConfigId in configurations) {
    configurations[recipesConfigId].forEach(function(rJsonInput) {
      ...
      renameJsonElement(rJsonInput, "computeWindowTimeInSeconds", "computeWindow", 20);
      renameJsonElement(rJsonInput, "locationUpdateIntervalSeconds", "reportingInterval", 5);
  ...
  if ("readerConfigurations" in configurations && "recipes" in configurations) {
    ...
    configurations["readerConfigurations"].forEach(function(rcJsonInput) {
      ...
      if(readerConfMap){
        ...
        console.warn(`Warning: Reader Configuration ${rcJsonInput.name} not used in recipe so can't converted to 2016r6 format.`);
      }
    });
  }
}
...
function convertReaderConfigurations(readerConfToRecipe, configurations){
  ...
  for(var readerName in readerConfToRecipe){
    ...
    if(currentReader.consistent == true){
      //console.log("Reader conf before:\n\t" + JSON.stringify(readerConfToRecipe[readerName].readerConf))
      ...
      if (readerConf["operation"] == "LOCATION" || readerConf["operation"] == "INVENTORY") {
