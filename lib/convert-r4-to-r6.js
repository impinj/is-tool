...
function convertReaderConfigurations(readerConfToRecipe, configurations){
  const xArrayAntennas = new Array(52).fill(1).map((c,i) => i+1);
  let errorList = [];
  for(let readerName in readerConfToRecipe){
    ...
    if(currentReader.consistent == true){
      ...
      if (currentReader.type === "LOCATION") {
        ...
        readerConf.configuration["disabledAntennas"] = xArrayAntennas.filter((item) => {
          return readerConf.configuration["antennas"].indexOf(item) === -1;
        });
