...
function convertRecipes(readerConfToRecipe, configurations) {
  ...
  if (globalConf.logging === 'loud') console.log('Converting recipes....');
...
function convertReaderDefinitions(configurations) {
  if ('readerDefinitions' in configurations) {
    for (const property in configurations.readerDefinitions) {
      if (Object.hasOwnProperty.call(configurations.readerDefinitions, property)) {
        const readerDefinition = configurations.readerDefinitions[property];
        if (readerDefinition.placement && readerDefinition.type === 'SPEEDWAY') {
          delete readerDefinition.placement;
        }
      }
    }
  }
...
function analyzeReaderConfToRecipesMap(readerConfToRecipe, configurations) {
  if ('readerConfigurations' in configurations && 'recipes' in configurations) {
    if (globalConf.logging === 'loud') console.log('Analyzing readerConfigurations....');
    for (const rcJsonInput in configurations.readerConfigurations) {
      if (Object.hasOwnProperty.call(configurations.readerConfigurations, rcJsonInput)) {
        ...
        if (readerConfMap) {
          ...
          if (currentReaderConf.tagPopulationEstimate === null) {
            currentReaderConf.tagPopulationEstimate = 32;
          }
...
function convertR4ToR6(inputConfigs) {
  if (!inputConfigs) throw new Error('Passed configuration object is null');

  const readerConfToRecipe = {};
  try {
    let configurations;
    if ((typeof inputConfigs) === 'string') {
      configurations = JSON.parse(inputConfigs);
    } else {
      configurations = inputConfigs;
    }

    convertRecipes(readerConfToRecipe, configurations);
    convertReaderDefinitions(configurations);
    ...
    if (globalConf.logging === 'loud') console.log(
